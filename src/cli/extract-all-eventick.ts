...
async function extractAllEventickEvents(): Promise<void> {
  logger.info('🚀 Iniciando extracción completa de eventos de Eventick');
  ...
  try {
    ...
    logger.info(`📁 Revisa el archivo eventick_all_events.csv para ver los ${eventIds.length} eventos`);
  } catch (error) {
