...
import { EventickRecordSchema, type EventickRecord } from '../../types/eventick';

const BASE_URL = 'https://eventick.com.ar';

const buildEventUrl = (eventId: number): string => {
  return `${BASE_URL}/evento/${eventId}`;
};

export const eventickScraper: SiteScraper<EventickRecord> = {
  siteName: 'eventick',
  recordSchema: EventickRecordSchema,
  ...
  async extractLinksFromList(page: Page, pageNum: number): Promise<string[]> {
    ...
    try {
      await navigateWithRetry(page, eventUrl, 2, 'networkidle2');
  ...
  async extractRecord(browser: Browser, url: string, timeoutMs: number): Promise<EventickRecord> {
      ...
      const data = await page.evaluate(() => {
        ...
        if (organizerPhone === 'No encontrado') {
          ...
          if (phoneLinks.length > 0) {
            const phoneLink = phoneLinks[0] as HTMLAnchorElement;
        ...
        if (organizerEmail === 'No encontrado') {
          ...
          if (emailLinks.length > 0) {
            const emailLink = emailLinks[0] as HTMLAnchorElement;
